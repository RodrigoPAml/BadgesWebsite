...
import { DeleteCompetence } from '../../services/Competences';
...
function DeleteContainer(props) {
  ...
  return <Card sx={{ m: '10px' }}>
    <Typography sx={{ color: 'rgba(0, 0, 0, 0.5)', m: '10px' }}>
      Você tem certeza que deseja deletar a competência "{get(props, 'item.description')}". Essa ação é irreversível.
    </Typography>
    <Box sx={{ display: 'flex', justifyContent: 'flex-end', m: '10px' }}>
      <Button
        onClick={() => {
          DeleteCompetence(get(props, 'item.id')).then((response) => {
